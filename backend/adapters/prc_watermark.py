...
class PrcWatermarkAdapter(ModelAdapter):
    def run(self, request: WatermarkRequest) -> WatermarkResult:
        ...
        if request.workflow not in {"generate", "detect"}:
            return WatermarkResult(job_id, request.method, request.workflow, "unsupported", 0, "PRC-Watermark exposes generation and keyed binary verification.", "real", None, ["The released repository does not implement candidate-bank identification."], {})
        ...
        if request.workflow == "generate":
            ...
            if not request.source_job_id:
                return WatermarkResult(job_id, request.method, request.workflow, "failed", 0, "missing decoding key", "real", None, ["PRC verification requires the decoding key saved by a compatible generation job."], {})
